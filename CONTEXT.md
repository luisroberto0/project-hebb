...
- ✅ **plasticidade local SEM backprop (premissa-mãe, §1 linha 16)** — **Marco 3 (eixo SoftHebb): 1º POSITIVO LIMPO (2026-06-18).** Pilha conv treinada só por Hebbiano competitivo local (zero backprop nas features) → **80.27% CIFAR-10** (linear-probe), **+11.67 p.p. sobre pesos-random** (sinal real, oposto do STDP), competição essencial (wta_off colapsa, oposto do C2), a **6.8 p.p. do backprop**, filtros = features reais que não colapsam. **Generaliza: margem ~constante +12 p.p. em CIFAR-10 E CIFAR-100.** SUCESSO em 2/3 sub-critérios; margem ~3 p.p. abaixo do limiar +15 (TODAS as 4 vias principled de empurrar esgotadas honestamente: mais-treino/whitening/profundidade/dificuldade). A premissa-mãe, finalmente testada limpa, FUNCIONA — o primeiro mecanismo bio-inspirado que carrega sinal genuíno. `experiment_06_plasticity/`

**Roadmap pós-Marco 3 (confirmado pelo Luis, 2026-06-18):**
- 🔵 **Marco 4 (ATIVO) — continual learning com plasticidade local:** o SoftHebb (features genéricas, online, sem backprop) resiste a catastrophic forgetting que o backprop sofre? Une o mecanismo que funciona a uma capacidade pós-LLM real; revisita o Marco 1 com a abordagem certa. Critério literal + protocolo: `experiment_07_continual_local/PLAN.md`.
- ⬜ **Marco 5 — escala (ImageNet):** até onde o SoftHebb vai (paper: 27% probe). Caracteriza limites.
- ⬜ **Marco 6 — hardware neuromórfico:** medir energia real em silício (Loihi/SpiNNaker via EBRAINS, ou Akida AKD1000). Onde a eficiência bio-inspirada se materializa. Requer acesso a hardware.
