...
\textbf{Main empirical findings.} (i) The four sparsity levels tested cross-domain converge tightly within a 0.52 p.p.\ range (k=8: 21.68\%; k=16: 22.09\%; k=32: 22.20\%; k=64 / vanilla: 22.13\%), with all 95\% confidence intervals overlapping. The 3.78 p.p.\ in-domain spread documented in~\citet{pinho2026kwta} \emph{collapses} cross-domain. (ii) An encoder with random untrained weights followed by k-WTA $k=16$ reaches 21.91\%---within the overlapping 95\% CIs of the trained encoders, indicating that source-domain training contributes essentially no transferable signal in this setting. (iii) Pixel kNN, with no encoder at all, reaches 22.81\%, with a confidence interval that does not overlap any of the trained encoders, suggesting that the CNN-4 forward pass discards more cross-domain information than it preserves (we examine likely mechanisms, including max-pooling, in Section~\ref{sec:discussion}). (iv) ProtoNet retrained directly on CUB at $28 \times 28$ grayscale reaches 34.31\%, and at $84 \times 84$ RGB reaches 49.84\%---establishing that target-domain training (+12.22 p.p.\ over source-trained) and adequate input resolution (+15.53 p.p.\ over the 28-pixel grayscale baseline) are the operative bottlenecks, neither of which sparsity addresses.
...
\begin{enumerate}
    ...
    \item We \textbf{document a k-WTA effect collapse}: the in-domain sparsity-accuracy spread of 3.78 p.p.\ is reduced to 0.52 p.p.\ cross-domain, within overlapping CIs. We use \emph{effect collapse} to denote this reduction of the measured spread, not a demonstrated causal mechanism.
    ...
    \item We \textbf{characterize anti-transfer} (the absence of measurable positive transfer): source-trained encoders fall within the overlapping CIs of random-weight controls in this regime.
\end{enumerate}

We do not claim a new method, biological realism, or a breakthrough mechanism. We claim a precise empirical statement, scoped to this single source--target pair and backbone (Omniglot$\to$CUB, CNN-4): under extreme domain shift between visually unrelated source and target, an effect that mattered in-domain disappears, and bio-inspired sparsity neither helps nor hurts.
